...
class LineDetect(Node):
	def __init__(self,name):
		...
		self.Mouse_XY = (0, 0)
		self.hsv_text = "src/yahboomcar_linefollow/yahboomcar_linefollow/LineFollowHSV.text"
	...
	def declare_param(self):
		...
		self.declare_parameter("LaserAngle",180)
	...
	def execute(self, point_x, color_radius):
		...
		if self.Joy_active == True:
			...
			return
		self.Start_state = True
		if color_radius == 0 and self.warning <100 and self.temp ==False:
			print("Not Found")
			self.pub_cmdVel.publish(Twist())
		elif self.warning <100 and self.temp ==True:
			twist = Twist()
			twist.angular.z = -0.8
			twist.linear.x = 0.2
			self.pub_cmdVel.publish(twist)
